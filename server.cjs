...
function detectSchemaType(schema) {
  ...
  // Try both valid JSON and lenient parsing (for unquoted type names like { name: String })
  if (isValidJSON(s) || (s.startsWith('{') && s.endsWith('}'))) {
    try {
      const result = detectAndConvertPlainType(s, 'AutoGenerated');
      if (result) {
        console.log('✓ Detected plain-types');
        return 'plain-types';
      } else {
        console.log('detectAndConvertPlainType returned null/falsy');
      }
    } catch (e) { 
      console.log('❌ detectAndConvertPlainType threw error:', e.message);
    }
  }
